...
is
   ...
   Jet : Plane :=
     (CockpitDoors => Open, ExternalDoors => Open, Ignition => Off, Tank => 100,
      Status       => Idle, FuelLight => GREEN, AltiLight => GREEN,
      SpeedLight   => GREEN, Height => 0, Velocity => 0, Wheels => Deployed);

   Eolo : Weather := (Flow);

   procedure TakingOff(eolo: in Weather) with
     Global => (In_Out => (Jet, File_System)),
     Pre    => Jet.Status = Idle and Jet.CockpitDoors = Locked and
   ...
   procedure Flying with
     Global => (In_Out => Jet),
     Pre    => Jet.Status = TakingOff and Jet.Velocity >= 300 and
     Jet.Height >= 500,
